...
function renderRoutes(routes) {
  ...
  routes.forEach((route) => {
    ...
    // adjusted_score: frontend-scaled 1–5 (relative)
    // raw_safety_score: pure ML score from backend
...
feedbackForm.addEventListener("submit", async (e) => {
  ...
  // For unsafe / moderate ratings → must give details
  if (rating <= 3) {
    ...
    // For 4–5, details optional; send 'none' if empty
...
weatherBtn.addEventListener("click", async () => {
  ...
  const lon = document.getElementById("weather-lon").value.trim();
  if (!lat || !lon) {
  ...
  try {
    ...
    weatherResult.innerHTML = `
      <div><strong>${data.name || "Location"}</strong></div>
